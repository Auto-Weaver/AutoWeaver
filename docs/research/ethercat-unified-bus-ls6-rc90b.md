...
> **本节已被薄翻译层方案超越**。本节写于 2026-05-12，当时的结论是"在 EVO-003 的 `MotionAxis + IoModule` 二分法基础上增补第三类 `RobotProgram`"。同日稍后的讨论中进一步认识到：**所有从站本质上都是"挂着一份契约的字段端点"，根本不需要类型分化**。最终方案是把 runtime 做成只懂"字段名 ↔ 字节"的薄翻译层，业务语义在 leaf，字节布局在外置 YAML 契约里。
>
> 现行设计见 [EVO-003: Rust Motion Runtime](../evo/003-motion-runtime.md)。本节内容保留作思考演进记录。
...
> **更新（同日，结论第二、五条已被超越）**：进一步讨论后，runtime 不再做"三类从站"的内部分化；改走**薄翻译层 + 外置 YAML 契约**路线——所有从站都是"挂着一份契约的字段端点"，runtime 代码不懂任何设备类型。现行设计见 [EVO-003](../evo/003-motion-runtime.md)。本节下方结论保留作思考演进记录。
