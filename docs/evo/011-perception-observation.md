...
> **修订 2 · 2026-07-27（observe 刀合并后）** —— 又推翻一条，这次是本文自己的核心论断之一：**"Sensor 对时钟被动 —— no internal heartbeat, no thread"**（§2.3）。
>
> **为什么倒了**：`observe()` 是 **pull**——调用方问一次给一张，节奏是"拍 → 处理 → 拍 → 处理"，**采集速率被消费者绑住**。慢 observer 甩出去（§5.5）不解决问题：快 observer 的耗时、加上调用方拿到帧之后自己做的事，全都堵在下一次 `observe()` 前面。而相机天然是 **push**，按自己帧率吐。要让采集不被消费拖住，就得让 Sensor 拥有自己的采集节奏——也就是一条线程。
>
> **实测证据**（`pluck/backend/src/workers/drill_vision.py:78-95`，0727 的 run）：PNG 压缩 **247ms** vs `capture()` **≈31ms**，消费是生产的 **8 倍**；循环服务时间 288ms/帧，而平移窗口只有 0.05–0.30s，于是每次抬升只采到 **1–3 帧**。该文件自己的结论：「**调 `lift_move_step_mm` 毫无用处**: 0.5mm 的门在 70–100mm/s 下每 5–7ms 就满足一次, 卡的是服务时间不是触发门槛」。pluck 为此长出 `drill-lift-stream` 线程 + 异步写盘才跑起来——**所以采集线程不是新增，是把业务层已经存在的线程收到该在的地方。**
>
> **当时错在哪**：本文把 `architecture.md:51` 的"任何 **Worker** 不得维持自己的心跳"**推广到了设备层**。Worker 是参与控制的那层（接 note、写 state、BT 从它读判据），Sensor 不是。真正承重的判据更窄，且完好无损：**采集线程不写 control state、不发 note、不参与判据**，它只把观测放进环。BT 仍是唯一的**控制**调度方。用户的框定：「BT 更多的是统一调度和保证时钟同步，大脑其实不完全关心手臂和四肢。」
>
> **范围后果（重要）**：§5.4 的有界环**从"下一轮的存储议题"提前为"连续采集的功能前提"**——采集线程一独立，环就是观测的落点，没有环采集线程就没地方放帧。它不再是内存优化。
>
> **连带结论**：两种模式都要支持——**按需一张**（pull，扫描位停稳拍照，flush 队列取最新）+ **连续流**（push，采集线程自己的节奏，慢消费者**丢帧**而不拖采集）。pluck 长出两套东西、还得手写"连拍时 `on_tick` 让路"，根子就是**框架只给了 pull**。
>
> 受影响章节：§2.3（论证保留、标注取代）、§5.4（性质变更）。**代码侧不变**：`observe()` 是 pull 模式的正确实现，作为"按需一张"保留，一行未改。设计留给 **EVO-012**（环的大小、丢帧策略、过期语义、两种模式并存），本文不下结论。
...
| 角色 / 物 | 名字 | 动作 | 说明 |
|---|---|---|---|
| 仪器 / 感知层**唯一对外门面** | `Sensor` | `observe()` | ~~对时钟被动~~（⛔ 已推翻，见 §2.3；连续模式下 Sensor 自有采集节奏），对观察者主动。知道自己的 role、成像条件、快门时刻。`observe` 串行化是**契约**。**时钟同步是硬责任**：`captured_at` 必须同一时钟，否则多相机观测与臂轨迹对不齐 |
...
> ⛔ **上面这句已被推翻**（2026-07-27，见顶部「修订 2」）。**保留原文不删**，因为它错得有价值：它对 Worker 是对的，对设备是错的，而这个区分正是要点。
>
> 简述：`observe()` 是 pull，采集速率被消费者绑住（pluck 实测消费 247ms vs 采集 31ms，8 倍），连续采集必须让 Sensor 自己有采集节奏。**当时的错误是把"Worker 不得维持自己的心跳"推广到了设备层。** 承重的那半句仍然成立且是 EVO-012 的硬判据：**采集线程不写 control state、不发 note、不参与判据。**
>
> **句中后半仍然正确**：`architecture.md:51` 约束的是 **Worker**，那条确实没被破坏——它从一开始就不管 Sensor。
...
| 直播 | 每 N tick 让它观测一次 |
| 按需一张 | 在该拍的时刻发一条 note |
| 突发（边动边采） | ~~`RepeatUntil` 循环节点（`EVO-010`），条件成立就一直观测~~ ⛔ **已推翻**：见下方说明，连续采集移入 Sensor（EVO-012）。**直播/按需两行仍然成立。** |
...
>
> ⛔ **这一整段已被推翻**（2026-07-27）。**保留不删，它是本文错得最实质的一段。**
>
> 三处都错了：
> 1. **那不是"存量违规"，那是这个问题的正确答案，只是长在了业务层。** pluck 被逼着建那条线程，是因为不解耦采集与消费就采不到帧。
> 2. **"在本模型下它就是一个 BT 循环"是错的。** BT 循环解决不了：50 Hz tick 贴不上相机帧率与位移门，而且——真正的缺陷——**pull 循环无论由哪条线程驱动，采集都串在消费后面**。
> 3. **"50 Hz tick 对 0.5 mm 步距绰绰有余"把因果搞反了。** 实测服务时间 288ms/帧、窗口 0.05–0.30s，卡的从来不是触发门槛。原文记着：「**调 `lift_move_step_mm` 毫无用处**」。
>
> 正确的说法是：那条线程该**下移到 Sensor**（EVO-012），而不是上移到 BT。判据是它不参与控制——不写 state、不发 note、不出判据。
...
> ⚠️ **性质已变更**（2026-07-27，见顶部「修订 2」）：有界环**不再是"下一轮的存储优化"，而是连续采集的功能前提**。采集线程一旦独立，环就是观测的落点——没有环，采集线程没地方放帧。所以它必须和 EVO-012 的采集线程一起定，不能再往后推。
>
> 这也改变了本节最后那句的适用范围：**"本轮不做记录所以不阻塞 `observe` 落地"对已合并的 pull 模式仍然成立**（`observe()` 同步返回，帧的归属是调用方），但**对连续模式不成立**——那一刀必须同时给出环的大小、丢帧策略和过期语义。
>
> **丢帧策略有业务含义，不是纯技术选择**：预览丢帧无所谓，录像丢帧是坏的，留档丢帧可能不可接受。EVO-012 要按消费者分别定。
...
> ⚠️ **解法已被 [EVO-012 §2.4](012-continuous-acquisition.md) 更正**（2026-07-28）：本节预设了"引用会失效"（因为假想的实现是「有界环 + 引用过期」）。EVO-012 改成**入队即拥有**——拒绝发生在 `submit()` 那一刻，收下之后永不失效，脏读因此**不存在**而不是"被处理好了"。**本节的动机仍然成立，解法以 EVO-012 为准。**
