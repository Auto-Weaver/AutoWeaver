...
### `read_until`
- 入参：`source` + `until`（判据，如 `equals: 0`）+ `timeout_s`（**必填**，两种合法取值见下）
- 行为：轮询读，直到判据满足返回；若声明了有限超时且到期，标记 error（交给 BT 决策）。
- 用于"等 PLC 把请求标志清零（= 动作完成）"、"等清洗回执"、将来"等开机自检回魔数"。

#### `timeout_s`：数字 = 有界等待，显式 `null` = 这一步没有超时语义

```yaml
- read_until: { register: plc_send, equals: CLEAR, timeout_s: 120 }   # 有界：到点抛 ReadUntilTimeout
- read_until: { register: plc_send, equals: SET,   timeout_s: null }  # 无界：等多久都等
```

原设计默认"等不到 = 异常"，但这不普适。在 PLC 驱动的链路上，**PC 等 PLC 发请求就是本分**：PLC 慢（上料、清洗、人工干预、单步调试）是正常工况，不是故障；而且 BT 收到这个 error 也没有可决策的内容——重试就是再等一轮，放弃就是停机。所以缺的是**表达能力**，不是分工要改。

补法是给既有原语的入参加一个合法取值，**不加第四个原语、不给原语加 mode 开关**（见铁律 1、§三个原语开头那句"想加 `mode:` 就是烂的信号"）。行为完全由声明的数据决定：契约里写什么，引擎就照着执行；同一份契约换个引擎，语义不该变。

**显式 `null` 是一次声明，省略是一次疏忽——两者绝不能长得一样。**所以 `timeout_s` 这个键**缺失时报 schema 错**（`ActionStepError`），不给默认值：忘了填就永远等，是最难查的一类 bug；而"我确认这一步不该有超时"必须在契约里留下可读、可 review 的字面痕迹。

`ReadUntilTimeout` 语义原样保留：声明了数字就照旧超时抛错交给 BT，一点不变。
