...
> 0.5.x → 0.6.0 的翻转点：Subsystem 改名为 Worker，BT 树从"可选编排"升级到"唯一主动调度方"。0.5.x 时（EVO-006）的描述见 [EVO-006（已废弃）](evo/006-superseded-bt-clock-and-subsystem.md)。
...
│     4. broadcast on_tick 给所有 RUNNING 的 Worker             │
└────────────────────────────────────────────────────────────┘
       │                                  │
       ▼ 推进                               ▼ 广播 tick
┌─ BT 树 ───────────────────────────┐  ┌─ Workers ───────────────────┐
...
| **Worker** | 持 Sensor + Pipeline + 跨帧状态；对外暴露 namespace；被 BT 通过 note 唤醒 | 业务侧实现（如任意 business Worker）|
| **BT 树** | 系统唯一主动调度方 | Control/Decorator + 三类 leaf |
| **BTClock** | 系统节拍 + 生命周期 + 异常隔离 | autoweaver 提供 |
| **WorldBoard** | 跨 Worker 的 state + note 通道 | autoweaver 提供 |

> Worker 在 0.8.x 拆为 `PerceptionWorker`（handler 返回即完成）/ `MotionWorker`（tick 边沿即完成）两个子基类，详见 [EVO-007](evo/007-bt-worker-task.md)。

**Task 仍然是一个概念**，但不再是顶层抽象——它是 Worker 内部的 *装配组件*（Pipeline + 跨帧状态的绑定），不再被某个 Engine.tick(data) 推。业务侧自由决定一个 Worker 里要不要拆 Task。
...
- **Namespace 硬约束**：Worker 只能写 `<self.name>.*` 下的 state；跨 namespace 读没有限制。
- **BT leaf 全无状态**：跨 tick 状态收敛在 Worker 内部或 motion runtime，leaf 只是"询问器"。
- **tick 顺序固定**：4 阶段顺序不变——BT 在 tick N pass 的 note 在 tick N+1 才被 Worker 收到。这个半 tick 延迟是有意为之的对齐机制。
- **异常隔离**：任何 Worker / leaf 抛异常被框架捕获，标记 FAULTED 不再接收 tick；其他模块不受影响。

## 不属于 autoweaver

- **具体业务逻辑**：Pipeline 里的 step、Worker 里的 Task 组合、BT 树拓扑 —— 都是调用方实现
- **部署 / 容器化 / 进程管理** —— 调用方决定
- **存储 / API 端** —— 调用方决定
